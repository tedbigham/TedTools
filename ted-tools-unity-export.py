bl_info = {
	...
	"author": "Angel 'Edy' Garcia (@VehiclePhysics), texture embedding additions by Ted Tools",
	"version": (1, 4, 2),
	"blender": (3, 0, 0),
	"location": "File > Export > Ted Tools Unity FBX",
	"description": "Unity-compatible FBX exporter with embedded textures.",
	"warning": "",
	"wiki_url": "",
...
def make_image_file_backed_and_packed(img, texture_dir):
    ...
    print(f"Saved and packed image for FBX embedding: {filepath}")


def prepare_images_for_embedding(filepath):
    texture_dir = os.path.join(os.path.dirname(filepath), "_GeneratedTextures")
...
# End texture embedding additions --------------------------------------------
...
def export_unity_fbx(context, filepath, active_collection, selected_objects, deform_bones, leaf_bones, primary_bone_axis, secondary_bone_axis, tangent_space, triangulate_faces):
	...
	# Ensure generated/in-memory images can be embedded by Blender's FBX exporter.
	prepare_images_for_embedding(filepath)
...
def menu_func_export(self, context):
	self.layout.operator(ExportUnityFbx.bl_idname, text="Ted Tools Unity FBX (.fbx)")
